...
export const verifyGst = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const create = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const company = await createCompany(req.user!, req.body as CreateCompanyInput);
...
export const update = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const company = await updateCompany(req.user!, req.params.id as string, req.body as UpdateCompanyInput);
    res.json({ success: true, data: toCamelCase(company) });
  } catch (error) {
    next(error);
  }
