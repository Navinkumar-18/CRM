...
export const list = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await getDeals(req.user!, page, limit, search, stage, companyId);
...
export const pipeline = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const update = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const deal = await updateDeal(req.user!, req.params.id as string, req.body as UpdateDealInput);
    res.json({ success: true, data: toCamelCase(deal) });
  } catch (error) {
    next(error);
  }
...
export const changeStage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const deal = await updateDealStage(req.user!, req.params.id as string, req.body as UpdateDealStageInput);
    res.json({ success: true, data: toCamelCase(deal) });
  } catch (error) {
    next(error);
  }
